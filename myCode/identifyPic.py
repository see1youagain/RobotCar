...
class identifyPic():
    def __init__(self):
        # self.cap = cv2.VideoCapture("http://192.168.8.44:8080/stream?topic=/camera/rgb/image_raw")
        # 读取模板图像
        template_A = []
        template_B = []
        template_C = []
    ...
    def template_matching(self, image_gray, template, threshold):
        res = cv2.matchTemplate(image_gray, template, cv2.IMREAD_GRAYSCALE)
    ...
    def getPic(self):
        ...
        # hx, image = cap.read()
        hx = True
        image = cv2.imread("D:/RobotCar/result1/alphabet/aera12.jpg")
        ...
        for i in range(len(template_A)):
            ...
            kindtmp=0
            kindtmp += 1 if len(locations_Atmp) > 0 else 0
            kindtmp += 1 if len(locations_Btmp) > 0 else 0
            kindtmp += 1 if len(locations_Ctmp) > 0 else 0
            if(kindtmp>=kind):
