...
class Main():
    ...
    def run(self):
        ...
        while not rospy.is_shutdown() and loopCount>0:
            ...
            if(self.arriveOKFlag==True and self.threeMinOKFlag==False and self.blankLoopFlag==False and self.roadWindow==-1 and self.roadNum==-1):
                self.blankLoopFlag=True
                self.dispense_pub.publish(1)
                self.cmd_pub.publish(1)
