...
class MoveBaseSquare():
    ...
    def __init__(self):
        ...
        euler_angles = (pi/2, pi/2 ,pi/2,-pi/2, -pi/2, -pi/2,-pi/2,-pi/15 , pi-pi/30 , pi , pi/2 , -pi/2 , 0 , -pi/2 , -pi/2)
        ...
        # 二车
        self.waypoints = list()
        self.waypoints.append(Pose(Point(3.257, 1.900, 1.533), quaternions[0]))  # //C
        self.waypoints.append(Pose(Point(2.494, 2.536, 1.484), quaternions[1]))  # //A
        self.waypoints.append(Pose(Point(3.332, 2.800, 1.520), quaternions[2]))  # //B
        self.waypoints.append(Pose(Point(0.803, 0.966, - 1.614), quaternions[3]))  # //4
        self.waypoints.append(Pose(Point(-0.000, 1.550, - 1.625), quaternions[4]))  # //3
        self.waypoints.append(Pose(Point(0.919, 1.880, - 1.614), quaternions[5]))  # //2
        self.waypoints.append(Pose(Point(0.081, 2.550, - 1.627), quaternions[6]))  # //1
        self.waypoints.append(Pose(Point(1.861, 0.112, - 0.032), quaternions[7]))  # 起点
        self.waypoints.append(Pose(Point(1.408, 3.944, 3.099), quaternions[8]))  # 终点
        self.waypoints.append(Pose(Point(2.201, 3.871, 2.264), quaternions[9]))  # 配药区到取药区的必经点
        self.waypoints.append(Pose(Point(2.863, 2.636, 1.539), quaternions[10]))  # 配药中点
        self.waypoints.append(Pose(Point(0.5, 2.705 , -1.539), quaternions[11]))  # 取药中点
        self.waypoints.append(Pose(Point(1.1, 0.262 , - 0.662), quaternions[12]))  # 取药区到起点的必经点
        self.waypoints.append(Pose(Point(0.065, 0.270, - 1.603), quaternions[13]))  # 仓库
        self.waypoints.append(Pose(Point(-1.840, -0.320, -0.100), quaternions[14]))  # 检查点
        ...
        while (not rospy.is_shutdown()):  # 如果ros系统没有关机的话
            ...
            if (self.count == 1 and self.flag <= 1):  # 从起点到配药区
            ...
            elif (self.count == 17 and self.flag <= 1): 
                rospy.loginfo("去检查点")
                # Intialize the waypoint goal
                # 初始化goal为MoveBaseGoal类型
                goal = MoveBaseGoal()
                ...
                # 设置时间戳
                goal.target_pose.header.stamp = rospy.Time.now()
                goal.target_pose.pose = self.waypoints[14]
                # Start the robot moving toward the goal
                # 机器人移动
                if (self.move(goal) == True):
                    rospy.sleep(0.5)
