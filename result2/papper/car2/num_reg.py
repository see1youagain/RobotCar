...
class identify_num():
    def __init__(self):
        ...
        # 遍历指定目录下所有子目录及模板图像
        for i in range(10):
            self.templates.extend(glob.glob('/home/EPRobot/robot_ws/src/eprobot_start/script/temps/' + str(i + 1) + '.jpg'))
    ...
    def get_number_img(self, img):
        ...
        _, contours, _hierarchy = cv2.findContours(bin, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        index = 0
        img_map = []
        Isget = 0
        # 轮廓遍历
        for cnt in contours:
            ...
            # print("cnt:{}  contourArea:{}  isContourConvex:{}".format(len(cnt),cv2.contourArea(cnt),cv2.isContourConvex(cnt)))
            if len(cnt) == 4 and cv2.contourArea(cnt) > 200 and cv2.contourArea(cnt) < 40000 and cv2.isContourConvex(cnt):
                # 计算轮廓的中心位置
    ...
    def getMatchResult(self):
        ...
        if Isget:
            ...
            if i == 0:
                ValueOfNumber = '7 5 4 1'
            if i == 1:
                ValueOfNumber = '4 3 2'
            if i == 2:
                ValueOfNumber = '7 8 5 6'
            if i == 3:
                ValueOfNumber = '1 7 6 5 9'
            if i == 4:
                ValueOfNumber = '8 0 5'
            if i == 5:
                ValueOfNumber = '4 2 3'
            if i == 6:
                ValueOfNumber = '7 1 9'
            if i == 7:
                ValueOfNumber = '8 0 6 4'
            if i == 8:
                ValueOfNumber = '0 3 8'
            if i == 9:
                ValueOfNumber = '8 6 0 9'
            # 清空历史值
            matchValue = []
            print(u'The Number is', ValueOfNumber)  # 测试语句
        # if ValueOfNumber == -1:
        #     cv2.imwrite("/home/EPRobot/robot_ws/src/eprobot_start/script/temps/1.jpg", img_fir)
