...
class IdentifyPic():
    def __init__(self):
        ...
        template_B = []
        template_C = []
        folder_pathA = '/home/EPRobot/robot_ws/src/eprobot_start/script/alphabet/A'
        filesA = os.listdir(folder_pathA)
        folder_pathB = '/home/EPRobot/robot_ws/src/eprobot_start/script/alphabet/B'
        filesB = os.listdir(folder_pathB)
        folder_pathC = '/home/EPRobot/robot_ws/src/eprobot_start/script/alphabet/C'
    ...
    def getPic(self):
        cap=cv2.VideoCapture("http://192.168.8.50:8080/stream?topic=/camera/rgb/image_raw")
        ...
        for i in range(len(template_A)):
            # 模板匹配
            locations_ATmp = self.template_matching(image_gray, template_A[i], threshold)
            locations_BTmp = self.template_matching(image_gray, template_B[i], threshold)
            locations_CTmp= self.template_matching(image_gray, template_C[i], threshold)

            if (len(locations_ATmp) >0 and len(locations_BTmp)>0 and len(locations_CTmp) >0):
                break
            locations_A = locations_A if len(locations_ATmp)<len(locations_A) else locations_ATmp
            locations_B = locations_B if len(locations_BTmp) < len(locations_B) else locations_BTmp
            locations_C = locations_C if len(locations_CTmp) < len(locations_C) else locations_CTmp
        if(len(locations_A) + len(locations_B)  + len(locations_C) >=3):
            ...
            cv2.imwrite("D:/RobotCar/myCode/alphabet/aera3.jpg", aera1_crop)
